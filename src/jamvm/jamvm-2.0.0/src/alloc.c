...
typedef struct nvmChunk
{
 ...
 static nvmChunk *nvm_free_list;
 static char *nvm;
 static int nvm_fd;
 static int nvm_header_size = sizeof(nvmChunk);
 static int min_size = (0x3 + sizeof(nvmChunk) + sizeof(void*));
 static unsigned int nvm_free_space = 0;
...
void addConservativeRoot(Object *object) {
    if((conservative_root_count % LIST_INCREMENT) == 0) {
        ...
        conservative_roots = gcMemRealloc(conservative_roots, new_size * sizeof(Object *), CON_ROOTS_NAME, FALSE);
